...
🚧 **Work in progress.** All five datasets — SAIPE (poverty rate + median household income), LAUS (unemployment rate), Eviction Lab (eviction filing rate), Food Access Atlas (food desert population share), and Vulcan CO2 (total emissions) — plus county boundaries are now live end-to-end, from fetch through Postgres/GeoJSON, and the pipeline is instrumented with OpenTelemetry. Four Grafana Geomap layers (poverty rate, unemployment rate, eviction filing rate, food desert population share) are built. Still to build: a CO2 Geomap layer. See [project.md](project.md) for the full brief, dataset list, and build sequence — the steps below reflect what's actually implemented today.
